...
describe("record-based cross-device convergence", () => {
  ...
  it("keeps a tombstone authoritative over a stale active robot", () => {
    let garage = upsertGarageEntry(
      createGarageManifest(deviceA, 1),
      {
      id: robotId,
      nickname: "Robot A",
  ...
  it("recovers preferences and presets from independent records", () => {
    ...
    const records = [preferencesToSyncRecord(settings), presetToSyncRecord(settings.presets[0])].map((record, index) =>
      decodeGarageRecordEvent(buildGarageRecordEvent(secret, record, 20 + index), secret)!
    );
    const recovered = recoverySnapshotFromRecords(secret, records, deviceB);
  ...
  it("recovers sanitized finished trades from independent history records", () => {
    const entry = tradeHistoryEntryFromOrder(
      {
        slotId: hexToBase91("c".repeat(64)),
    ...
    const event = decodeGarageRecordEvent(buildGarageRecordEvent(secret, record, 30), secret)!;
